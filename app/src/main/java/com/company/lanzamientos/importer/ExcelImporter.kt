...
object ExcelImporter {
    ...
    suspend fun importWorkbook(context: Context, uri: Uri, repo: Repo) = withContext(Dispatchers.IO) {
        context.contentResolver.openInputStream(uri).use { stream ->
            ...
            val purchased = mutableSetOf<Pair<String,String>>()
            ...
            wb.forEach { sheet ->
                ...
                val isVendorSheet = headers.any { it.equals("Cliente2", true) || it.equals("Cliente", true) } && headers.any { it.equals("Localidad", true) }
                val looksProductSheet = headers.firstOrNull()?.equals("COD", true) == true && sheet.sheetName.any { it.isDigit() }
                ...
                if (isVendorSheet) {
                    ...
                    val productCols = headers.mapIndexedNotNull { i, h ->
                        if (i in skip || h.isBlank()) null else {
                            ...
                            repo.upsertProduct(code, name); products += code; i to code
                        }
                    }
                    (1..sheet.lastRowNum).forEach { r ->
                        val row = sheet.getRow(r) ?: return@forEach
                        val cod = clean(row.getCell(idxCOD)?.toString()); if (cod.isBlank()) return@forEach
                        ...
                        repo.ensureAssignment(cod, sellerId); clients += cod
                    ...
                    repo.upsertProduct(productCode, productCode); products += productCode
                    val idxCOD = headers.indexOfFirst { it.equals("COD", true) }.takeIf { it >= 0 } ?: 0
                    ...
                    (1..sheet.lastRowNum).forEach { r ->
                        ...
                        if (flag.isNotBlank()) { purchased += cod to productCode; clients += cod }
                    }
                }
            }

            val allClients = (repo.getAllClientIds().toSet() + clients)
            val allProducts = (repo.getAllProductCodes().toSet() + products)
